;;; package-manager.el --- Setup package manager system -*- lexical-binding: t; -*-
...
(let* ((archive-dir (expand-file-name "elpa/archives/gnu/archive-contents" user-emacs-directory))
  ...
  (if (or (not package-archive-contents) ; if no cache
      ...
      (progn
	(message "ELPA cache is deprecated. Updating index...")
	(package-refresh-contents))
        (message "ELPA cache is up-to-date (< %d days)." days-between-updates)))
...
(when (version< emacs-version "29.1")
  (unless (package-installed-p 'use-package)
	...
	(package-install 'use-package)
      (error
       (message "Failed upon installing use-package. Updating ELPA index...")
       (package-refresh-contents)
       (package-install 'use-package)))))
...
(add-hook 'use-package-ensure-failed-hook
	  (lambda (package error)
	    (message "Failed installing %s due to: %s. Trying to update ELPA..." package error)
	    (package-refresh-contents)

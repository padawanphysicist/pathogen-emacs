...
;; integrates it with use-package. This module must load before any
;; configuration that uses external packages.
;;
;;; Code:
...
(add-hook 'elpaca-after-init-hook
          (lambda ()
            (message "[Pathogen] Emacs loaded in %s with %d garbage collections."
                     (format "%.2f seconds"
                             (float-time
